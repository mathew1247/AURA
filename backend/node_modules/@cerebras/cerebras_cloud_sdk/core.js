...
class AbstractPage {
    ...
    async getNextPage() {
        ...
        if (!nextInfo) {
            throw new error_1.CerebrasError('No next page expected; please check `.hasNextPage()` before calling `.getNextPage()`.');
        }
...
const validatePositiveInteger = (name, n) => {
    if (typeof n !== 'number' || !Number.isInteger(n)) {
        throw new error_1.CerebrasError(`${name} must be an integer`);
    }
    if (n < 0) {
        throw new error_1.CerebrasError(`${name} must be a positive integer`);
    }
    return n;
};
...
const ensurePresent = (value) => {
    if (value == null)
        throw new error_1.CerebrasError(`Expected a value to be given but received ${value} instead.`);
    return value;
};
...
const coerceInteger = (value) => {
    ...
    throw new error_1.CerebrasError(`Could not coerce ${value} (type: ${typeof value}) into a number`);
};
...
const toBase64 = (str) => {
    ...
    throw new error_1.CerebrasError('Cannot generate b64 string; Expected `Buffer` or `btoa` to be defined');
};
